...
impl DataTypeTrait<MyGraphState> for MyDataType {
    fn data_type_color(&self, _user_state: &MyGraphState) -> egui::Color32 {
        match self {
            MyDataType::WaveTable => egui::Color32::from_rgb(38, 109, 211),
    ...
    fn name(&self) -> Cow<'_, str> {
        match self {
            MyDataType::WaveTable => Cow::Borrowed("scalar"),
...
impl NodeTemplateTrait for MyNodeTemplate {
    ...
    fn build_node(
        ...
        let output_scalar = |graph: &mut MyGraph, name: &str| {
            graph.add_output_param(node_id, name.to_string(), MyDataType::WaveTable);
        };
